...
/** 파일 여러 개를 받아 영수증으로 만들고, **곧바로 읽는다.**
 *
 * 예전에는 넣기만 하고 [영수증 읽기]를 따로 누르게 했는데, 넣으면 금액이 채워질 것으로
 * 여기는 것이 자연스럽다는 지적을 받아 바꿨다. 어차피 넣은 뒤 할 일은 읽는 것뿐이다.
 *
 * 열지 못하는 파일은 조용히 흘리지 않고 무엇이 왜 안 되는지 알린다.
 * 브라우저는 아이폰 기본 형식(HEIC)을 대개 열지 못한다 — 윈도우판은 열 수 있어
 * 두 판의 차이가 여기서 드러난다. */
async function 들이기(파일들) {
  const 온것 = [...파일들];
  if (!온것.length) return;

  알림(`영수증 ${온것.length}장 준비 중…`);
  const 새로넣은것 = [];
  const 못연것 = [];
  for (const f of 온것) {
    try {
      ...
      if (넣을자리에(영)) 새로넣은것.push(영);
    } catch {
      못연것.push(f.name || "이름 없는 파일");
    }
  }
  그리기();
  ...
  if (못연것.length) {
    alert(
      ...
      "아이폰 사진(HEIC)이면 이렇게 해 주세요.\n" +
      "  · 설정 → 카메라 → 포맷 → '높은 호환성' 으로 바꿔 다시 찍기\n" +
      "  · 또는 사진을 카카오톡·메일로 한 번 보내 JPG 로 받은 뒤 넣기\n\n" +
      "윈도우 프로그램은 HEIC 도 읽습니다.");
  }
  if (새로넣은것.length) await 판독하기(새로넣은것);   // 넣자마자 읽는다
}

/** 빈자리가 있는 첫 쪽에 넣는다. 한 쪽이 6장을 넘지 않게 한다.
 *  넣었으면 true, 자리가 없어 못 넣었으면 false. */
function 넣을자리에(영) {
  let 번호 = 1;
  while (번호 <= 최대섹션수) {
    const s = 결.항목들[번호 - 1];
...
/** 넘겨받은 영수증들을 차례로 읽어 금액·날짜를 채운다.
 *
 * 영수증을 넣을 때 자동으로 불리고, [다시 읽기] 를 누르면 전부를 다시 읽는다.
 * 한 장이 실패해도 멈추지 않고 다음 장으로 넘어간다 — 한 장 때문에 전체가
 * 날아가면 사용자는 무엇이 문제인지 알 수 없다. */
async function 판독하기(것들) {
  if (!것들.length || 읽는중) return;
  읽는중 = true;
  그리기();

  if (!준비됐나()) {
    알림("판독기를 준비하는 중입니다. 처음 한 번은 한국어 데이터(12MB)를 내려받느라 좀 걸립니다…");
  }
